...
adminRouter.get("/requests", async (_req, res) => {
  ...
  const metaByRequestId = new Map(
    negotiationMeta.map((entry) => [String(entry._id), entry])
  );

  const mergedRequests = requests.map((request) => {
    const meta = metaByRequestId.get(String(request._id));

    return {
      ...request,
      latestNegotiation: meta?.latestNegotiation,
      negotiationCount: meta?.negotiationCount ?? 0
    };
  });
